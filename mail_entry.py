...
title = f"data/title_{datetime.now().strftime('%d-%m-%Y')}.txt"
...
class UserDataEntry(QMainWindow):
    def __init__(self):
        ...
        # Load title from file if it exists
        try:
            with open(title, 'r') as title_file:
                title_text = title_file.read()
                self.title_entry.setText(title_text)
    ...
    def save_to_excel(self, entries):
        ...
        print("Data saved to 'user_data.xlsx'.")
    ...
    def collect_entries(self):
        ...
        with open(title, 'w') as title_file:
...
class DisplayWindow(QMainWindow):
    ...
    def close_windows(self):
        ...
        print("[IDENTIFIER] sentmail.py")
        sys.stdout.flush()
        time.sleep(1)
        subprocess.run(["python", "sentmail.py"])
